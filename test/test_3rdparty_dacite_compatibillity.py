...
def test_dacite_rejects_wrong_types():
    ...
    with pytest.raises(WrongTypeError):
        from_dict(Root, {'Training': {'Optimizer': {'lr': 'oops'}, 'epochs': 5}, 'seed': 42}, config=DaciteConfig(strict=True))

    with pytest.raises(WrongTypeError):
        from_dict(RootSnake, {'training': {'optimizer': {'lr': 'oops'}, 'epochs': 5}, 'seed': 42}, config=DaciteConfig(strict=True))
