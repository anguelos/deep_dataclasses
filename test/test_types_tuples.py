...
def test_tuple_of_dataclasses():
    ...
    c = Config(plugins=[Config.Plugin(name='bar'), Config.Plugin(name='other bar'), Config.Plugin(name='last bar')])
...
def test_unparameterized_tuple_of_dataclasses():
    ...
    assert len(c.plugins) == 3
    assert c.plugins[-1].name == 'last bar'
    c = Config(plugins=[Config.Plugin(name='bar'), Config.Plugin(name='other bar')])
...
def test_unparameterized_pep585tuple_of_dataclasses():
    @deep_dataclass
    class Config:
        @auxiliary
        class Plugin:
